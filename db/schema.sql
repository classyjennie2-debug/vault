...
-- Referral codes generated by users for sharing
...
-- Tracks new users who signed up via referral
...
-- Referral bonuses earned when referred user deposits
CREATE TABLE IF NOT EXISTS referral_bonuses (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  referrer_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  referral_id UUID NOT NULL REFERENCES referrals(id) ON DELETE CASCADE,
  deposit_transaction_id UUID NOT NULL REFERENCES transactions(id) ON DELETE CASCADE,
  deposit_amount DECIMAL(15,2) NOT NULL,
  bonus_amount DECIMAL(15,2) NOT NULL,
  bonus_percentage DECIMAL(5,2) DEFAULT 10.0, -- Default 10% bonus
...
-- Separate balance tracking for referral earnings
...
-- Tracks transfers from referral balance to main balance
CREATE TABLE IF NOT EXISTS referral_withdrawals (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  amount DECIMAL(15,2) NOT NULL,
  referral_balance_before DECIMAL(15,2) NOT NULL,
  referral_balance_after DECIMAL(15,2) NOT NULL,
...
-- Trigger for referral_balance updated_at
DROP TRIGGER IF EXISTS update_referral_balance_updated_at ON referral_balance;
CREATE TRIGGER update_referral_balance_updated_at BEFORE UPDATE ON referral_balance
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Trigger for referral_bonuses updated_at
DROP TRIGGER IF EXISTS update_referral_bonuses_updated_at ON referral_bonuses;
CREATE TRIGGER update_referral_bonuses_updated_at BEFORE UPDATE ON referral_bonuses
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Trigger for referral_withdrawals updated_at
DROP TRIGGER IF EXISTS update_referral_withdrawals_updated_at ON referral_withdrawals;
CREATE TRIGGER update_referral_withdrawals_updated_at BEFORE UPDATE ON referral_withdrawals
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
